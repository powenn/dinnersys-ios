...
class HistoryTableViewController: UITableViewController {
    ...
    private func fetchData(){
        ...
        print(today)
        historyTableList = []
            let balanceParam: Parameters = ["cmd":"get_pos"]
            AF.request(dsRequestURL, method: .post, parameters: balanceParam).responseData{ balanceResponse in
                ...
                do{
                    POSInfo = try decoder.decode(CardInfo.self, from: balanceResponse.data!)
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let unpaidAction = UIAlertAction(title: "取消訂單", style: .destructive, handler: {(action: UIAlertAction!) -> () in
            AF.request("\(dsURL("delete_self"))&order_id=\(info.id!)").responseData{response in
                if response.error != nil {
                    let errorAlert = UIAlertController(title: "Bad Internet.", message: "Please check your internet connection and retry.", preferredStyle: .alert)
                    errorAlert.addAction(UIAlertAction(title: "OK", style: .default, handler: {
                        (action: UIAlertAction!) -> () in
        ...
        let paymentAction = UIAlertAction(title: "以學生證付款(餘額：\(balance))", style: .default, handler: {(action: UIAlertAction!) -> () in
            ...
            let sendAction = UIAlertAction(title: "確認", style: .default, handler: { (action: UIAlertAction!) -> () in
                ...
                //if(false){
                    ...
                    AF.request("\(dsURL("payment_self"))&target=true&order_id=\(info.id!)&password=\(payment_pw)").responseData{ response in
